...
Datorro::Datorro(float samplerate) : m_dryWet(-0.5), m_samplerate(samplerate) {
    ...
    ((Onepole*)filters[11])->setCoefficinets(((Onepole*)filters[11])->makeHighPass(1000, m_samplerate));
    filters[12] = new Onepole(m_samplerate);
    ((Onepole*)filters[12])->setCoefficinets(((Onepole*)filters[12])->makeHighPass(1000, m_samplerate));

    filters[1] = new Allpass(0.75, 2100, m_samplerate);
    filters[2] = new Allpass(0.75, 1580, m_samplerate);
    filters[3] = new Allpass(0.625, 5610, m_samplerate);
    filters[4] = new Allpass(0.625, 4100, m_samplerate);
    filters[9] = new Allpass(0.5, 39310, m_samplerate);
    filters[10] = new Allpass(0.5, 26640, m_samplerate);

    filters[5] = new ModAllPass(0.7, 13430, m_samplerate, 0.1, 12);
    filters[6] = new ModAllPass(0.7, 9950, m_samplerate, 0.03, 12);
