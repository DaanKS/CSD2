...
public:
  ...
  float calculateTheta(float cutoffFreq);
  float calculateAlpha(float theta);
  float calculateCoefficients();

private:
  inline void calculateBzero(){Bzero = ((1.0 - cos(theta)) / 2.0); }
  inline void calculateBone(){Bone = (1.0 - cos(theta));}
  inline void calculateBtwo(){Btwo = ((1.0 - cos(theta)) / 2.0);}
  inline void calculateAzero(){Azero = 1.0 + alpha;}
  inline void calculateAone(){Aone = -2.0 * cos(theta);}
